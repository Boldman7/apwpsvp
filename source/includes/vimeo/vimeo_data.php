...
if($type == 'video_comments'){
	...
	$result = $vimeo->request("/channels/$path/videos", array(
													'page'=> $page,
													'per_page' => $perPage,
													'fields' => 'uri,name,description,link,duration,release_time,privacy,pictures.sizes,stats.plays,metadata.connections.comments.total,metadata.connections.likes.total,categories.uri,categories.name,categories.top_level,user.uri,user.name,user.link,user.pictures.sizes,user.account',
													'sort' => $sort,
													'direction' => $sortDirection,								
													'filter' => 'embeddable',
	...
	$result = $vimeo->request("/users/$user_id/albums/$path/videos", array(
													'page'=> $page,
													'per_page' => $perPage,
													'fields' => 'uri,name,description,link,duration,logo,release_time,privacy,pictures.sizes,stats.plays,metadata.connections.comments.total,metadata.connections.likes.total,categories.uri,categories.name,categories.top_level,user.uri,user.name,user.link,user.pictures.sizes,user.account',
													'sort' => $sort,
													'direction' => $sortDirection,							
													'filter' => 'embeddable',
													'filter_embeddable' => 'true'
